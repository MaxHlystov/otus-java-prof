...
public class ATM {
    ...
    private final TreeMap<BankNote, AtmCell> cells = new TreeMap<>();
    ...
    public Map<BankNote, Integer> getSum(final BigDecimal sum) {
        ...
        while (currentSum.compareTo(BigDecimal.ZERO) > 0) {
            AtmCell floorCell = getFloorEntry(currentSum);
            if (floorCell == null) {
                throw new IllegalArgumentException("We cannot cashing sum " + sum);
            }
            BankNote currentBankNote = floorCell.getBankNote();
            if (floorCell.isEmpty()) {
                cells.remove(currentBankNote);
                continue;
            }
            int take = floorCell.takeBanknotesToSum(currentSum);
            if (take == 0) {
                throw new IllegalArgumentException("We cannot cashing sum " + sum);
            }
            if (floorCell.isEmpty()) {
    ...
    private AtmCell getFloorEntry(BigDecimal sum) {
        BankNote fake = new BankNote(sum);
        var floor = cells.floorEntry(fake);
        if (floor == null) {
            return null;
        }
